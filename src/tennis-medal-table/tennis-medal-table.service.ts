// src/tennis-medal-table/tennis-medal-table.service.ts
import { Injectable } from '@nestjs/common';
import { DataSource } from 'typeorm';
import {
  TennisMedalRow,
  TennisMedalSummaryResponse,
} from './dto/medal-summary-response.dto';
import { TennisMedalDetailResponse, TennisMedalDetailRow } from './dto/medal-detail-response.dto';

@Injectable()
export class TennisMedalTableService {
  constructor(private readonly dataSource: DataSource) {}

  async getMedalSummary(
    externalEventId: number,
    localSportId: number,
  ): Promise<TennisMedalSummaryResponse> {
    // Cubre tanto Tenis de Campo como Tenis de Mesa —
    // el localSportId es el único diferenciador entre ambos.
    const rows: Array<{
      phaseId: number;
    ...
    let currentRank = 1;
    const general: TennisMedalRow[] = sorted.map((row, idx) => {
      if (idx > 0) {
  ...
  async getMedalDetailByInstitution(
    ...
    localSportId: number,
    institutionId: number,
  ): Promise<TennisMedalDetailResponse> {
    const rows: Array<{
      phaseId: number;
      phaseName: string;
      registrationId: number;
    ...
    const athletes: TennisMedalDetailRow[] = rows
      .filter((row) => Number(row.participantCount) >= 2)
      .map((row) => {
        const pos = Number(row.position);
        const medalType: TennisMedalDetailRow['medalType'] =
