// src/wushu-medal-table/wushu-medal-table.service.ts
import { Injectable } from '@nestjs/common';
import { DataSource } from 'typeorm';
import {
  WushuMedalRow,
  WushuMedalSummaryResponse,
} from './dto/medal-summary-response.dto';
import { WushuMedalDetailResponse, WushuMedalDetailRow } from './dto/medal-detail-response.dto';

@Injectable()
export class WushuMedalTableService {
  constructor(private readonly dataSource: DataSource) {}

  async getMedalSummary(
    externalEventId: number,
    localSportId: number,
  ): Promise<WushuMedalSummaryResponse> {
    const rows: Array<{
      phaseId: number;
    ...
    let currentRank = 1;
    const general: WushuMedalRow[] = sorted.map((row, idx) => {
      if (idx > 0) {
  ...
  async getMedalDetailByInstitution(
    ...
    localSportId: number,
    institutionId: number,
  ): Promise<WushuMedalDetailResponse> {
    const rows: Array<{
      phaseId: number;
      phaseName: string;
      registrationId: number;
    ...
    const athletes: WushuMedalDetailRow[] = rows
      .filter((row) => Number(row.participantCount) >= 2)
      .map((row) => {
        const pos = Number(row.position);
        const medalType: WushuMedalDetailRow['medalType'] =
